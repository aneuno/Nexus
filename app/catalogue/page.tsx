  ...
  return (
    <main style={{ minHeight: '100vh', background: '#0a0a14', color: '#e8e0cc', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ padding: '20px' }}>
        {loading ? (
          <div style={{ textAlign: 'center', padding: '4rem', color: 'rgba(201,168,76,0.5)' }}>
            Chargement...
          </div>
        ) : filtered.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: '12px' }}>
            {filtered.map(card => (
              <div key={card.id} onClick={() => setSelected(card)} style={{ cursor: 'pointer', borderRadius: '8px', overflow: 'hidden', transition: 'all 0.2s' }}>
                <div style={{ width: '100%', aspectRatio: '0.72', borderRadius: '8px', overflow: 'hidden', background: '#141428', border: '1px solid ' + rc(card.rarity) + '60', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {card.image_url ? (
    ...
    🌀 Voir en 3D
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {selected.description && (
                  <div style={{ fontSize: '0.8rem', color: 'rgba(232,224,204,0.6)', lineHeight: '1.6', borderTop: '1px solid rgba(201,168,76,0.15)', paddingTop: '12px', marginBottom: '10px' }}>{selected.description}</div>
                )}
                {selected.effect && (
                  <div style={{ fontSize: '0.8rem', color: 'rgba(232,224,204,0.75)', lineHeight: '1.6', padding: '10px 12px', background: 'rgba(201,168,76,0.04)', borderRadius: '4px', border: '1px solid rgba(201,168,76,0.12)' }}>{selected.effect}</div>
                )}
              </div>
            </div>
          </div>
